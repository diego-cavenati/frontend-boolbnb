    ...
    watch: {
        dataLoaded(newValue) {
            if (newValue) {
                ...
                const apartments = store.results;
                const locations = [];

                // Filtra gli appartamenti visibili e crea un array con le loro coordinate
                for (const apartment of apartments) {
                    if (apartment.visible === 1) {
                        const location = { longitude: apartment.longitude, latitude: apartment.latitude };
                        locations.push(location);
                    }
                }

                // Calcola le coordinate univoche dei punti sulla mappa
                const uniqueLocations = [];
                for (const location of locations) {
                    const filteredLocations = uniqueLocations.filter((l) => l.longitude === location.longitude);

                    if (filteredLocations.length === 0) {
                        uniqueLocations.push(location);
                    } else {
                        let sameLatLocations = filteredLocations.filter((l) => l.latitude === location.latitude);

                        if (sameLatLocations.length === 0) {
                            uniqueLocations.push({ longitude: location.longitude, latitude: location.latitude + 0.009 });
                        } else {
                            const increment = sameLatLocations.length * 0.009;
                            uniqueLocations.push({ longitude: location.longitude, latitude: location.latitude + increment });
                        }
                    }
                }

                // Aggiungi un offset casuale alle coordinate per evitare sovrapposizioni di punti
                const result = [];
                const latMap = new Map();
